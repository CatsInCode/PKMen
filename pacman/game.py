...
class Game:
    ...
    def __restart_current_scene(self) -> None:
        ...
        if isinstance(scene_manager.current, MainScene):
            ...
            new_scene.set_pacmans_visible(self.__render_pacmans)
            self.__apply_display_mode()
            return
        ...
        for cmd in control_panel.read_commands():
            if cmd.name == "ghost_count":
                ...
                if isinstance(scene_manager.current, MainScene):
                    ...
                    scene_manager.current.set_pacmans_visible(self.__render_pacmans)
            elif cmd.name == "show_paths":
                self.__show_paths = bool(cmd.value)
                if isinstance(scene_manager.current, MainScene):
                    scene_manager.current.set_paths_visible(self.__show_paths)
            elif cmd.name == "render_pacmans":
                self.__render_pacmans = bool(cmd.value)
                if isinstance(scene_manager.current, MainScene):
                    scene_manager.current.set_pacmans_visible(self.__render_pacmans)
